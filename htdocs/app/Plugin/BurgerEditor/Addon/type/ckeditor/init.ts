...
// tslint:disable-next-line:no-mergeable-namespace no-namespace
declare namespace CKEDITOR {
	/* CKEDITOR v4.5.2 */
	export const stylesSet: CKEDITOR.resourceManager;
	export const protectedSource: RegExp[];
	export interface resourceManager {
		add(name: string, definition?: {}): void;
		get(name: string): {};
	}
}

interface ICkeditorTypeContentData extends IBurgerTypeContentData {
	ckeditor: string;
}

interface ICkeditorTypeCustomData {
	editor: CKEDITOR.editor | null;
}

BgE.registerTypeModule<ICkeditorTypeContentData, ICkeditorTypeCustomData>('Ckeditor', {
	data: {
		editor: null,
	},
	open: (editorDialog, type) => {
		...
		const defaultConfig: CKEDITOR.config = {
			toolbar: [
				['Source', 'ShowBlocks'],
				['Templates'],
		...
		const config: CKEDITOR.config = $.extend({}, defaultConfig, BgE.config.ckeditorConfig);

		const dtd: any = CKEDITOR.dtd;
		dtd.a.address = 1;
		dtd.a.article = 1;
		dtd.a.aside = 1;
		dtd.a.audio = 0;
		...
		(CKEDITOR.config as any).stylesCombo_stylesSet = CKEDITOR_STYLE_SET_NAME;
		...
		const cke = CKEDITOR.replace($editor.get(0) as HTMLTextAreaElement, config);
	...
	beforeExtract: (editorDialog, type) => {
