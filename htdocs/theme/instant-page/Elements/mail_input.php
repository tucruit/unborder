	...
	foreach ($mailFields as $key => $record) {
		...
		if ($field['use_field'] && ($blockStart && $iteration >= $blockStart) && (!$blockEnd || $iteration <= $blockEnd)) {
			...
			if ($group_field != $field['group_field'] || (!$group_field && !$field['group_field'])) {
				...
				if ($field['not_empty']) {
					echo '<span class="mod-form-hissuTag">' . __('必須') . '</span>';
				} else {
			...
			/* 入力欄 */
			if ((!$freezed || $this->Mailform->value("MailMessage." . $field['field_name']) !== '') && (!empty($field['before_attachment']))) {
			...
			if ((!$freezed || $this->Mailform->value("MailMessage." . $field['field_name']) !== '') && (!empty($field['after_attachment']))) {
				echo '<span class="bs-mail-after-attachment">' . $field['after_attachment'] . '</span>';
			}
			if (!$freezed && (!empty($field['attention']))) {
